    ...
    try {
        ...
        if (buyerListings.length === 0) {
            ...
            for (const listing of buyerListings) {
                ...
                if (listing.status === ListingStatus.MATCHED && listing.otpBuyer) {
                    message += `Your OTP: ${listing.otpBuyer}\n`;

                    if (listing.travelerConfirmed) {
                        message += `✅ Traveler has confirmed delivery\n`;
                    } else {
                        message += `⏳ Waiting for traveler confirmation\n`;
                    }
                }
            }
        }
