...
import type {
  Bid as BotBidType,
  BotContext,
  Listing as BotListingType,
} from "./types.js"; // Renamed to avoid conflict
...
export async function startHandler(ctx: BotContext) {
  ...
  // Check if this is an authentication request from the web app
  // @ts-ignore - startPayload is available in Telegraf context but not in the type definition
  const startParam = ctx.startPayload;
  if (startParam && startParam.startsWith("auth")) {
    console.log(`Received authentication request with payload: ${startParam}`);

    // Create a deep link back to the web app with the auth data
    const appUrl = process.env.WEB_APP_URL || "http://127.0.0.1:3000";
    const authUrl = `${appUrl}/api/auth/telegram/callback?source=bot&telegram_id=${user.telegramId}`;
    ...
    await ctx.reply(
      `🔐 Authentication request received!\n\nClick the button below to complete your login:`,
      {
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [[{ text: "🔑 Complete Login", url: authUrl }]],
        },
      }
    );
...
export async function newRequestHandler(ctx: BotContext) {
  ...
  await ctx.reply(
    "Let's create a new delivery request! What would you like to have delivered? (Please describe the item, size, and any special requirements)"
  );
  ...
  try {
    ...
    for (const listing of availableDbListings) {
      ...
      const estimatedPrice =
        listing.estimatedPrice || details.estimatedPrice || "N/A";
...
`;
        ...
        Markup.inlineKeyboard(
          [
            ...
            [
              Markup.button.callback(
                `💲 Place Bid`,
                `bid_${listing.listingId}`
              ),
              Markup.button.callback(
                `✅ Accept Request`,
                `accept_${listing.listingId}`
              ),
            ],
    ...
    await ctx.reply(
      "Sorry, I couldn't fetch available requests right now. Please try again later."
    );
  }
}
...
export async function acceptHandler(ctx: BotContext) {
  await ctx.reply(
    'To accept a delivery request, please use the "Accept" buttons when viewing available requests with /available command.\n\nThis provides a more convenient way to accept requests without needing to type request IDs.'
  );

  // This function is no longer used as handlers were replaced with inline buttons
  return await ctx.reply(
    'To accept a delivery request, please use the "Accept" buttons when viewing available requests.'
  );
}
...
export async function statusHandler(ctx: BotContext) {
  ...
  try {
    ...
    const userListings = await ListingService.getListingsByUserId(
      dbUser.telegramId
    );
    ...
    const acceptedByMeListings = allListings.filter(
      (listing) =>
        listing.acceptedBy &&
        listing.acceptedBy.userId === dbUser.telegramId &&
    ...
    if (userListings.length === 0 && acceptedByMeListings.length === 0) {
      await ctx.reply(
        "You don't have any active requests or deliveries at the moment. Use /newrequest to create one or /available to find deliveries."
      );
      return;
    }
    ...
    if (userListings.length > 0) {
      await ctx.reply(
        `📤 Your outgoing requests (${
          userListings.filter(
            (l) => l.status !== "completed" && l.status !== "canceled"
          ).length
        } active):`
      );
      for (const listing of userListings) {
        if (listing.status === "completed" || listing.status === "canceled")
          continue; // Skip completed/canceled for brevity

        const details = listing.requestDetails as any;
        let requestCard = `📦 Item: ${
          details.item || "Not specified"
        } (ID: ${listing.listingId.substring(0, 6)})
📊 Status: ${formatStatus(listing.status)}`;
        ...
        if (listing.status === "pending") {
          ...
          requestCard += `\n🚚 Accepted by: ${
            listing.acceptedBy?.userName || "Courier"
          }`;
          requestCard += `\n🔑 Your OTP: ${listing.requesterOtp || "N/A"}`;
          buttons.push(
            Markup.button.callback(
              "✅ Verify Delivery",
              `verify_${listing.listingId}`
            )
          );
        }
      ...
      await ctx.reply(
        "You haven't created any requests yet. Use /newrequest to create one."
      );
    }
    ...
    if (acceptedByMeListings.length > 0) {
      await ctx.reply(
        `📥 Your accepted deliveries (${acceptedByMeListings.length}):`
      );
      for (const listing of acceptedByMeListings) {
        const details = listing.requestDetails as any;
        const requestCard = `🔹 Delivery for ${
          listing.userName
        } (ID: ${listing.listingId.substring(0, 6)})
...
🔑 Your OTP: ${listing.courierOtp || "N/A"}`;
        ...
        if (
          listing.pickupLocation &&
          (listing.pickupLocation.latitude || listing.pickupLocation.address)
        ) {
          buttons.push(
            Markup.button.callback(
              "📍 View Pickup",
              `courier_location_${listing.listingId}`
            )
          );
        }
        // @ts-ignore
        if (
          listing.dropoffLocation &&
          (listing.dropoffLocation.latitude || listing.dropoffLocation.address)
        ) {
      ...
      await ctx.reply(
        "You haven't accepted any deliveries yet. Use /available to find some."
      );
    }
  } catch (error) {
    console.error("Error fetching status:", error);
    await ctx.reply(
      "Sorry, I couldn't fetch your status right now. Please try again later."
    );
  }
}
...
export async function locationHandler(ctx: BotContext) {
  ...
  if (ctx.session.conversationState === "filling_form") {
    ...
    if (step === "pickup_location") {
      ...
      await ctx.reply(
        "Got your pickup location! Now, please share the dropoff location."
      );
...
Is there anything else you'd like to add about this delivery request?`);
    ...
    await ctx.reply(
      "Thanks for sharing your location! Here are nearby requests:"
    );
...
export async function handleTextMessage(ctx: BotContext) {
  ...
  if (
    ...
    if (!currentUser || !currentUser.telegramId) {
      await ctx.reply(
        "Sorry, I couldn't identify you. Please try /start again to resolve this."
      );
    ...
    try {
      ...
      if (!listing || listing.status !== "accepted") {
        await ctx.reply(
          "This delivery is not active or cannot be verified at this moment."
        );
        ctx.session.conversationState = "idle";
        ctx.session.verificationContext = undefined;
        return;
      ...
      if (isRequester) {
        // Current user is the requester, verifying courier's OTP
        if (listing.userId !== currentUser.telegramId) {
          await ctx.reply(
            "Verification mismatch. You are not the requester for this delivery."
          );
          ctx.session.conversationState = "idle";
          ctx.session.verificationContext = undefined;
          return;
        ...
        successMessageToCurrentUser =
          "✅ OTP verified successfully! Delivery marked as complete from your side.";
        successMessageToOtherParty = `✅ The requester (${
          currentUser.username || currentUser.firstName
        }) has verified your OTP for "${
          (listing.requestDetails as any).item
        }". Delivery complete!`;
      } else {
        // Current user is the courier, verifying requester's OTP
        if (listing.acceptedBy?.userId !== currentUser.telegramId) {
          await ctx.reply(
            "Verification mismatch. You are not the assigned courier for this delivery."
          );
          ctx.session.conversationState = "idle";
          ctx.session.verificationContext = undefined;
          return;
        ...
        successMessageToCurrentUser =
          "✅ OTP verified successfully! Delivery marked as complete from your side.";
        successMessageToOtherParty = `✅ The courier (${
          currentUser.username || currentUser.firstName
        }) has verified your OTP for "${
          (listing.requestDetails as any).item
        }". Delivery complete!`;
      ...
      if (otpToMatch === inputOtp) {
        ...
        if (otherPartyUserId) {
          try {
            await ctx.telegram.sendMessage(
              otherPartyUserId,
              successMessageToOtherParty
            );
          } catch (notifyError) {
            console.error(
              `Failed to send verification completion to ${otherPartyRole} ${otherPartyUserId}:`,
              notifyError
            );
        ...
        await ctx.reply(
          "❌ Invalid OTP. Please check and try again, or use /status to see details."
        );
      }
    } catch (dbError) {
      console.error(
        "Error during OTP verification database operation:",
        dbError
      );
      await ctx.reply(
        "Sorry, a database error occurred during verification. Please try again."
      );
    }
  ...
  if (ctx.session.currentBidding) {
    ...
    const listing = ctx.session.availableListings.find(
    ...
    try {
      ...
      if (isNaN(bidAmount)) {
        await ctx.reply(
          `Please enter a valid number for your bid amount. Minimum bid: $${minPrice}`
        );
        return;
      }
    } catch (error) {
    ...
    if (bidAmount < minPrice) {
      await ctx.reply(
        `Your bid of $${bidAmount} is below the minimum bid of $${minPrice}. Please enter a higher bid.`
      );
      return;
    }
...
Use /status to check your bid status.`);
    ...
    await ctx.reply(
      `[DEMO] Message sent to requester: "A new bid of $${bidAmount} has been placed on your request for "${
        listing.details.item
      }" by ${
        ctx.from?.username || "a courier"
      }. Check your status to view bids."`
    );
...
async function handleFormFillingConversation(ctx: BotContext, text: string) {
  ...
  switch (step) {
    case "item_details":
      ...
      try {
        const parsedData = await parseFormInput(
          "delivery_request",
          text,
          formData
        );
        ...
        // Move to next step
        await ctx.reply(
          "Great! Now please share your pickup location by sending your location or typing an address."
        );
        formData.step = "pickup_location";
      } catch (error) {
        await ctx.reply(
          "I couldn't understand the details. Please try describing the item again, including size and any special requirements."
        );
      }
      break;
    ...
    case "pickup_location":
      if (text.toLowerCase() === "use current location") {
        await ctx.reply(
          "Please use the location button to share your current location."
        );
      } else {
        // In a real app, you would geocode this address
        formData.pickupLocation = { address: text, latitude: 0, longitude: 0 };
        await ctx.reply(
          "Got your pickup address! Now, please share the dropoff location or address."
        );
    ...
    case "dropoff_location":
      ...
      await ctx.reply(
        `Great! I've got both locations. Is there anything else you'd like to add about this delivery request?`
      );
    ...
    case "confirmation":
      ...
      if (!ctx.dbUser || !ctx.dbUser.telegramId) {
        await ctx.reply(
          "Critical error: User information is missing. Cannot create request. Please /start again."
        );
        ctx.session.conversationState = "idle";
        ctx.session.formData = {};
        return;
      ...
      > = {
        ...
        // @ts-ignore
        userName:
          ctx.dbUser.username || ctx.dbUser.firstName || "Anonymous User",
      ...
      try {
        ...
        await ctx.reply(
          `✅ Your delivery request has been created! (ID: ${
            createdListing.listingId
          })

📦 Item: ${createdListing.requestDetails.item}
📍 Pickup: ${
            createdListing.pickupLocation?.address ||
            (createdListing.pickupLocation
              ? "Shared location"
              : "Not specified")
          }
🏁 Dropoff: ${
            createdListing.dropoffLocation?.address ||
            (createdListing.dropoffLocation
              ? "Shared location"
              : "Not specified")
          }
💲 Est. Price: ${
            createdListing.estimatedPrice ||
            (createdListing.requestDetails as any).estimatedPrice ||
            "N/A"
          }

Use /status to check your request status. We'll notify you when someone accepts it!`,
          Markup.keyboard([["/status", "/newrequest"], ["/available"]]).resize()
        );
      } catch (error) {
        console.error("Failed to save new listing to DB:", error);
        await ctx.reply(
          "Sorry, there was an error creating your request. Please try again."
        );
    ...
    default:
      await ctx.reply(
        "I'm not sure what information you're providing. Please use one of the commands like /newrequest to get started."
      );
  }
}
